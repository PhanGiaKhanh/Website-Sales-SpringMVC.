						...
						<div class="carousel-inner">
							<c:if test="${newProducts.size() > 0}">
								<c:forEach var="item" items="${newProducts}" varStatus="loop">
										${loop.first ? '<div class="item active"><ul class="thumbnails">': ''}
										${(loop.index)!=0 && (loop.index)%4==0? '</ul></div><div class="item"><ul class="thumbnails">': ''}
											<li class="span3">
										<div class="thumbnail">
											<a class="zoomTool" href="product_details.html"
										...
										${(loop.index+1) >= newProducts.size()? '</ul></div>': ''}
									</c:forEach>
							</c:if>
							<div class="item">
								<ul class="thumbnails">
	...
	-->
				...
				<div class="row-fluid">
					<c:if test="${products.size() > 0}">
						<c:forEach var="item" items="${products}" varStatus="loop">
							${loop.first ? '<ul class="thumbnails">': ''}
							${(loop.index)!=0 && (loop.index)%3==0? '</ul><ul class="thumbnails">': ''}
							<li class="span4">
								<div class="thumbnail">
									<a class="zoomTool" href="product_details.html"
										title="add to cart"><span class="icon-search"></span>
										QUICK VIEW</a> <a href="product-details/${item.productId}"><img
										src="<c:url value="/assets/user/img/${item.img}" />" alt=""></a>
									<div class="caption">
										<h5>${item.name}${loop.index+1}</h5>
										<h4>
											<a class="defaultBtn" href="product_details.html"
												title="Click to view"><span class="icon-zoom-in"></span></a>
											<a class="shopBtn" href="<c:url value="/addCart/${item.productId}" />" title="add to cart"><span
			...
			<div class="well well-small">
				<a class="btn btn-mini pull-right" href="#">Xem thêm <span
					class="icon-plus"></span></a> Xem tất cả sản phẩm
			</div>
		</div>
	</div>
